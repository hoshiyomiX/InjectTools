...
pub fn export_results(results: &[ScanResult], domain: &str) -> anyhow::Result<()> {
    ...
    let subdomain_issues: Vec<_> = results
        .iter()
        .filter(|r| {
            !r.is_working && !r.is_restricted && r.error_source.as_deref() == Some("subdomain")
        })
        .collect();
    let target_issues: Vec<_> = results
        .iter()
        .filter(|r| {
            !r.is_working && !r.is_restricted && r.error_source.as_deref() == Some("target")
        })
        .collect();
    ...
    if !restricted.is_empty() {
        content.push_str(&format!(
            "⚠ {} bug(s) with restrictions - connection works, try specific endpoints\n",
            restricted.len()
        ));
    }
    ...
    if !target_issues.is_empty() {
        content.push_str(&format!(
            "🎯 {} subdomain(s) OK but target offline - try different target domain\n",
            target_issues.len()
        ));
    }
    ...
    if working.is_empty() && restricted.is_empty() {
        ...
        content.push_str(&format!(
            "   Try: Different target domain or subdomain source\n"
        ));
    }
    ...
    println!(
        "{} {}",
        "Path:".bright_black(),
        filepath.display().to_string().bright_black()
    );
    println!("\n{}", "Summary:".bold());
    println!(
        "  {} {} working",
    ...
    if !target_issues.is_empty() {
        println!(
            "  {} {} target issues",
            "🎯".yellow(),
            target_issues.len().to_string().yellow()
        );
    }
...
pub fn view_results() -> anyhow::Result<()> {
    ...
    crate::ui::print_header("RECENT RESULTS");
    println!(
        "\n{} {} files\n",
        "Total:".bright_black(),
    ...
    for (idx, entry) in files.iter().enumerate().take(20) {
    ...
    if files.len() > 20 {
        println!("\n... and {} more files", files.len() - 20);
    }

    println!(
        "\n{} {}",
        "Directory:".bright_black(),
        results_dir.display().to_string().bright_black()
    );
